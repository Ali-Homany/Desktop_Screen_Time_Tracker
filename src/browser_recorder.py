...
"""
This module is responsible for recording the user's activity on the browser,
by receiving requests from the chrome extension.
"""
...
curr_domain_name = get_last_browser_tab(db) or 'BLANK'
curr_url = ''
...
unique_websites_names = set(get_all_websites_names(db))
# icons path
websites_icons_dir = os.path.join(
    os.path.expanduser('~'),
    'Documents',
    'Screen_Time_Tracker',
    'Websites_Icons'
...
def save_new_website(url: str, domain_name: str) -> bool:
    try:
        extract_website_icon(
            ...
            output_path=websites_icons_dir
        )
...
def record_active_browser_tab() -> None:
    ...
    if curr_domain_name not in unique_websites_names:
        ...
        threading.Thread(
            target=save_new_website,
            kwargs={'url': curr_url, 'domain_name': curr_domain_name}
        ).start()
